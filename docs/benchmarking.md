...
Because Grin can report `no_sync` before the node is fully caught up, benchmark completion is based on worker height being close to the gateway height rather than treating `no_sync` as complete.
