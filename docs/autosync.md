...
Completion is detected by comparing worker local block height to gateway local block height and validating the local chain through the owner API. The local block height is read from `sync_info.current_height`; `tip.height` is only a fallback when `sync_info.current_height` is unavailable. A worker is considered complete when it is API-up, has peers, its block height is within `RUNTIME_SYNC_COMPLETE_LAG` blocks of `grin-gw`, and `validate_chain` succeeds.
